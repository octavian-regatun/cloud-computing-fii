...
export const addCategoryToBook = async (req: IncomingMessage, res: ServerResponse) => {
  try {
    ...
    const existingRelation = await db.select().from(bookCategories).where(
      and(
...
export const removeCategoryFromBook = async (req: IncomingMessage, res: ServerResponse) => {
  try {
    const body = await getRequestBody(req);
...
export const getCategoriesByBookId = async (req: IncomingMessage, res: ServerResponse, bookId: number) => {
  try {
    ...
    const bookCategoryRelations = await db.select().from(bookCategories).where(eq(bookCategories.bookId, bookId));
    ...
    const categoryIds = bookCategoryRelations.map(relation => relation.categoryId);
    const bookCategories = await db.select().from(categories).where(
      categories.id.in(categoryIds)
    );
    
    res.writeHead(200);
    res.end(JSON.stringify({ data: bookCategories }));
  } catch (error) {
...
export const getBooksByCategoryId = async (req: IncomingMessage, res: ServerResponse, categoryId: number) => {
  try {
    ...
    const bookCategoryRelations = await db.select().from(bookCategories).where(eq(bookCategories.categoryId, categoryId));
    ...
    const categoryBooks = await db.select().from(books).where(
      books.id.in(bookIds)
    );
